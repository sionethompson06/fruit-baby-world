// POST /api/github/save-storybook-audio-script
// Saves storybookAudioScript on the episode JSON in GitHub.
// Auth: Protected by proxy.ts — requires valid admin cookie.
// Only updates the storybookAudioScript field; all other episode data is preserved.
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!isRecord(body.storybookAudioScript)) {
    return Response.json({ ok: false, message: "storybookAudioScript is required and must be an object." }, { status: 400 });
  }
  ...
  const normalizedScript: StorybookAudioScript = normalizeStorybookAudioScript(
    body.storybookAudioScript,
    storybookPages,
    featuredCharacters
  );
  normalizedScript.updatedAt = now;

  const episodeTitle = typeof episode.title === "string" ? episode.title : episodeSlug;
  const commitMessage = `Update audio script: ${episodeTitle}`;

  const updatedEpisode: Record<string, unknown> = {
    ...episode,
    storybookAudioScript: normalizedScript,
    updatedAt: now,
  };
  ...
  try {
    ...
    if (!putRes.ok) {
      ...
      console.error(`[save-storybook-audio-script] GitHub PUT failed (${putRes.status}):`, errBody);
    ...
    return Response.json({ ok: false, message: "Failed to commit audio script to GitHub." }, { status: 502 });
  }
}
