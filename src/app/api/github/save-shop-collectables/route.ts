...
export async function POST(request: Request): Promise<Response> {
  ...
  const normalized = normalizeShopCollectablesConfig({ ...body.config, updatedAt: now });

  const filePath = "src/content/shop/collectables.json";
  ...
  try {
    ...
    if (!putRes.ok) {
      ...
      console.error(`[save-shop-collectables] GitHub PUT failed (${putRes.status}):`, errBody);
      return Response.json(
        {
          ok: false,
          status: "github_error",
    ...
    return Response.json(
      { ok: false, status: "github_error", message: "Failed to commit shop collectables config to GitHub." } satisfies SaveResult,
      { status: 502 }
    );
  }
}
