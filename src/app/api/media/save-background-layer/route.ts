// POST /api/media/save-background-layer
// Uploads a background-only draft image to Vercel Blob and attaches the
// metadata to the episode scene's backgroundLayers[] in GitHub JSON.
//
// Auth:    Protected by proxy.ts — requires valid admin cookie.
// Safety:  Background layers are admin-only and never public.
...
// Phase:   18D.10.2 — Save Background Layer to Episode
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (typeof body.imageBase64 === "string" && body.imageBase64.trim().length > 0) {
    ...
    try {
      ...
      return Response.json(
        {
          ok: false,
          status: "missing_background_image",
          message: "imageBase64 could not be decoded. Regenerate the background draft and try again.",
        } satisfies SaveResult,
        { status: 400 }
      );
    }
    ...
    // Fetch external URL and re-upload to Blob for asset ownership
        ...
        return Response.json(
          {
            ok: false,
            status: "missing_background_image",
            message: `Could not fetch background image from provider URL (HTTP ${fetchRes.status}). Regenerate and try again.`,
          } satisfies SaveResult,
          { status: 400 }
        );
      }
      ...
      return Response.json(
        {
          ok: false,
          status: "missing_background_image",
          message: "Could not fetch background image from provider URL. The URL may have expired — regenerate and try again.",
        } satisfies SaveResult,
        { status: 400 }
      );
    }
  ...
  const storagePath = `episodes/${episodeSlug}/background-layers/scene-${sceneNumber}-${Date.now()}.${ext}`;

  console.info(
    `[save-background-layer] Uploading ${mimeType} (~${Math.round(imageBuffer.length / 1024)}KB) via ${sourceType} to ${storagePath}`
  );
  ...
  try {
    ...
    const safeMsg = (() => {
      ...
      if (err instanceof BlobFileTooLargeError)
        return "Background image is too large for Vercel Blob. Try regenerating.";
      if (err instanceof BlobError) return `Vercel Blob error: ${err.message}`;
      return "Blob upload failed. Check storage configuration.";
    })();
    console.error("[save-background-layer] Blob upload failed:", err instanceof Error ? err.message : err);
    return Response.json(
  ...
  try {
    ...
    if (!getRes.ok) {
      console.error(`[save-background-layer] GitHub GET failed (${getRes.status})`);
      return Response.json(
        {
          ok: false,
    ...
    console.error("[save-background-layer] Network error fetching episode:", err instanceof Error ? err.message : err);
    return Response.json(
      {
        ok: false,
        status: "github_save_failed",
  ...
  const backgroundLayer: EpisodeSceneBackgroundLayer = {
    ...
    settingLabel: typeof body.settingLabel === "string" ? body.settingLabel : undefined,
    environmentDescription:
      typeof body.environmentDescription === "string"
        ? body.environmentDescription.slice(0, 500)
        : undefined,
  ...
  const commitMessage = `Add background layer: ${episodeTitle} scene ${sceneNumber}`;
  ...
  try {
    ...
    if (!putRes.ok) {
      ...
      console.error(`[save-background-layer] GitHub PUT failed (${putRes.status}):`, errBody.slice(0, 200));
      return Response.json(
        {
          ok: false,
          status: "github_save_failed",
    ...
    console.info(`[save-background-layer] Saved background layer to ${filePath}, scene ${sceneNumber}`);
    ...
    return Response.json(
      {
        ...
        htmlUrl,
        notes: [
          "Background layer saved as admin-only scene asset.",
    ...
    console.error("[save-background-layer] Network error committing file:", err instanceof Error ? err.message : err);
    return Response.json(
      {
        ok: false,
        status: "github_save_failed",
