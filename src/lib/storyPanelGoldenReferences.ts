...
// Added for compatibility with harmonize-story-panel-draft route.

type GoldenSelectionResult = {
...
export function selectGoldenReferencesForHarmonization(opts: {
  ...
  const { all, characterSlugs, limit = 1, useGoldenReferences = true } = opts;
  ...
  if (!useGoldenReferences) {
    return {
      ...
      roles: [],
      mode: "none",
      skippedReason: "disabled by admin",
      matchReasons: [],
    };
  }
  ...
  const eligible = all.filter((r) => {
    ...
    const role = r.role ?? "";
    if (!HARMONIZATION_ROLES.has(role)) return false;
    const overlap = intersectCount(r.characterSlugs, characterSlugs);
    if (characterSlugs.length >= 2) return overlap >= Math.ceil(characterSlugs.length / 2);
    return overlap > 0 || role === "style-polish";
  });
  ...
  return {
    ...
    roles: selected.map((r) => r.role ?? ""),
    mode: selected.length > 0 ? "prompt-guided" : "none",
    matchReasons: selected.map(() => "harmonization match"),
  };
}
...
export function buildGoldenReferencePromptSection(
  ...
  const lines: string[] = [
    headers[context],
    GOLDEN_REFERENCE_PROMPT_HEADER,
    "",
  ];

  for (const ref of references) {
    const desc = (ref.qualityNotes || ref.notes || "").slice(0, 80);
    const descPart = desc ? ` — ${desc}` : "";
    const tags = (ref.settingTags ?? []).slice(0, 3).join(", ");
    const tagPart = tags ? ` [${tags}]` : "";
    lines.push(`• ${ref.title} [${ref.role ?? "style"}]${descPart}${tagPart}`);
  }

  const raw = lines.join("\n");
  return raw.length > DEFAULT_PROMPT_SECTION_LENGTH ? raw.slice(0, DEFAULT_PROMPT_SECTION_LENGTH) : raw;
}

export function buildGoldenReferenceReplayDiagnostics(
  result: GoldenSelectionResult,
  enabled: boolean
  ...
  if (result.count === 0 && result.consideredCount === 0) {
    ...
    warnings.push(`${result.consideredCount} Golden Reference(s) exist but none matched this scene.`);
  }
